...
#  Версия:    8.0 (All 23 improvements from audit)
...
# [IMP-12] Rollback stack
declare -a ROLLBACK_ACTIONS=()

# [IMP-2] Step dependency graph
declare -A STEP_DEPS=(
    [preflight]=""
    [update]="preflight"
    [deps]="update"
...
setup_logging() {
    ...
    if [ "${BASH_VERSINFO[0]}" -gt 4 ] || { [ "${BASH_VERSINFO[0]}" -eq 4 ] && [ "${BASH_VERSINFO[1]}" -ge 4 ]; }; then
        TEE_PID=$!; else TEE_PID=""; fi
    LOGGING_ACTIVE=true; msg_info "Лог: ${LOG_FILE}"
}
flush_log() {
...
is_done() {
    ...
    [ "$v" != "$SCRIPT_VERSION" ] && { msg_dim "${s}: v${v}→v${SCRIPT_VERSION}"; return 1; }; return 0
}
...
# [IMP-3] Show progress of all steps
show_progress() {
    [ "$SILENT" = true ] && return
    local done_count=0
    separator
...
# [IMP-9] Ask to continue on non-fatal error
...
require_disk_space() {
    ...
    if [ "$available_mb" -lt "$required_mb" ]; then
        ...
        apt-get clean 2>/dev/null; journalctl --vacuum-size=50M 2>/dev/null
        command -v docker &>/dev/null && docker system prune -f 2>/dev/null
...
validate_ip() {
    ...
    for x in "${o[@]}"; do [[ "$x" =~ ^[0-9]+$ ]] || return 1; [ "$x" -gt 255 ] && return 1; [[ "$x" =~ ^0[0-9] ]] && return 1; done
    [ "${o[0]}" = "0" ] && return 1; [ "$ip" = "255.255.255.255" ] && return 1; return 0
}
...
restore_os_release() {
    if [ -f "${BACKUP_DIR}/os-release.symlink" ]; then
        ...
        cp "${BACKUP_DIR}/os-release.original" "$lt" 2>/dev/null; ln -sf "$lt" /etc/os-release 2>/dev/null
...
run_wizard() {
    ...
    if [ "$OPT_STATIC_IP" = true ]; then
        ...
        while true; do
            ...
            validate_ip "$STATIC_IP" && break; whiptail --title "Ошибка" --msgbox "Неверный IP" 8 40; done
        [ "$OPT_STATIC_IP" = true ] && {
            STATIC_GW=$(whiptail --title "Шлюз" --inputbox "Шлюз:" 10 50 "$cgw2" 3>&1 1>&2 2>&3) || STATIC_GW="$cgw2"
            STATIC_DNS=$(whiptail --title "DNS" --inputbox "DNS:" 10 50 "8.8.8.8,1.1.1.1" 3>&1 1>&2 2>&3) || STATIC_DNS="8.8.8.8,1.1.1.1"; }
    fi
...
step_preflight() {
    ...
    if is_armbian; then
        is_pkg_installed armbian-zram-config && [ "$OPT_ZRAM" = true ] && { msg_warn "armbian-zram-config конфликт"; wrn=$((wrn+1)); }; fi
    # [IMP-13]
...
step_configure_network() {
    ...
    if who 2>/dev/null | grep -q pts; then
        msg_warn "SSH-сессия! Переключение сети через 10с..."
        sleep 10
    fi
    systemctl list-unit-files networking.service &>/dev/null && systemctl is-active --quiet networking 2>/dev/null && systemctl disable networking 2>/dev/null
...
step_configure_apparmor() {
    ...
    if [ "$aa" = "Y" ]; then msg_ok "AppArmor активен"
        ...
        for f in /boot/armbianEnv.txt /boot/uEnv.txt /boot/extlinux/extlinux.conf; do
            ...
            else grep -q "^extraargs=" "$f" && sed -i 's|^extraargs=.*|& apparmor=1 security=apparmor|' "$f" || echo "extraargs=apparmor=1 security=apparmor" >> "$f"; fi
...
step_performance() {
    ...
    elif command -v cpupower &>/dev/null; then cpupower frequency-set -g schedutil 2>/dev/null || cpupower frequency-set -g ondemand 2>/dev/null; msg_ok "CPU: schedutil"
    elif command -v cpufreq-set &>/dev/null; then echo 'GOVERNOR="schedutil"' > /etc/default/cpufrequtils; msg_ok "CPU: schedutil"
    else for g in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do [ -f "$g" ] && { echo schedutil > "$g" 2>/dev/null || echo ondemand > "$g" 2>/dev/null; }; done; fi
    # eMMC tuning
    if [ "$OPT_EMMC_TUNING" = true ]; then
        echo "vm.swappiness=10" > /etc/sysctl.d/99-ha-swap.conf; sysctl -p /etc/sysctl.d/99-ha-swap.conf >/dev/null 2>&1
        ...
        mkdir -p /etc/systemd/journald.conf.d; is_armbian && is_pkg_installed armbian-ramlog || {
        ...
        [ -n "$rd" ] && [ "$(cat "/sys/block/${rd}/queue/rotational" 2>/dev/null)" = "0" ] && {
            [[ "$rd" == nvme* ]] && echo none > "/sys/block/${rd}/queue/scheduler" 2>/dev/null
            [[ "$rd" == mmcblk* || "$rd" == sd* ]] && echo mq-deadline > "/sys/block/${rd}/queue/scheduler" 2>/dev/null; }
        msg_ok "eMMC tuning"
    fi
    # USB power
    [ "$OPT_USB_POWER" = true ] && { for d in /sys/bus/usb/devices/*/power/autosuspend; do [ -f "$d" ] && echo -1 > "$d" 2>/dev/null; done
        ...
        udevadm control --reload-rules 2>/dev/null; msg_ok "USB fix"; }
    mark_done "$sid"
}

# [IMP-5] Docker from official repo
step_install_docker() {
    ...
    command -v docker &>/dev/null && msg_ok "Docker: $(docker --version 2>/dev/null | awk '{print $3}' | tr -d ',')" || {
        ...
        detect_system_info
        local codename="${CACHED_CODENAME}"; [[ "$codename" == "trixie" ]] && codename="bookworm"
...
step_install_ha() {
    ...
    local -a _ps=("${PIPESTATUS[@]}"); local de=${_ps[0]}; set -o pipefail
...
DROPIN
    ...
    local sw=0; while ! systemctl is-active --quiet hassio-supervisor 2>/dev/null; do sleep 5; sw=$((sw+5))
        [ $sw -ge 120 ] && { msg_warn "Таймаут"; break; }; [ $((sw%15)) -eq 0 ] && msg_dim "${sw}с..."; done
...
step_security() {
    ...
    if [ "$OPT_UFW" = true ]; then any=true
        ...
        for r in "22/tcp SSH" "8123/tcp HA" "4357/tcp ESPHome" "5353/udp mDNS" "5683/udp HomeKit"; do
            local port="${r%% *}"; ufw status 2>/dev/null | grep -q "$port" || ufw allow "$port" comment "${r#* }" >/dev/null 2>&1; done
...
U
        ...
        systemctl enable fail2ban 2>/dev/null; systemctl restart fail2ban 2>/dev/null; msg_ok "Fail2Ban"
    fi
    if [ "$OPT_SSH_HARDENING" = true ]; then any=true; mkdir -p /etc/ssh/sshd_config.d; cp /etc/ssh/sshd_config "${BACKUP_DIR}/sshd_config.bak" 2>/dev/null
        ...
        systemctl list-unit-files ssh.service &>/dev/null && systemctl reload ssh 2>/dev/null || systemctl reload sshd 2>/dev/null; msg_ok "SSH"; fi
...
U
        printf 'APT::Periodic::Update-Package-Lists "1";\nAPT::Periodic::Unattended-Upgrade "1";\nAPT::Periodic::AutocleanInterval "7";\n' > /etc/apt/apt.conf.d/20auto-upgrades; msg_ok "Автообн."; fi
...
step_extras() {
    ...
    # [IMP-6] ha-notify with encrypted token via sed
...
TGEOF
    ...
    # [IMP-14] Watchdog with exponential backoff
...
if [ "$c" = "000" ]; then
    ...
    if [ "$fails" -ge 3 ] && [ "$mins_since" -ge "$backoff" ]; then
        docker restart homeassistant 2>/dev/null; /usr/local/bin/ha-notify "⚠️ WD #${fails} (${backoff}m)"
...
S
        chmod +x /usr/local/bin/ha-{watchdog,cleanup,net-recovery}; msg_ok "Watchdog (exp.backoff)"; fi
...
REOF
        chmod +x /usr/local/bin/ha-{backup,restore}; msg_ok "Бэкап"
    fi

    # [IMP-17] Remote backup
...
case "\$REMOTE" in
    ...
    *) /usr/local/bin/ha-notify "⚠️ Неизвестный протокол удалённого бэкапа";;
esac
RBEOF
        chmod +x /usr/local/bin/ha-backup-remote; msg_ok "Удалённый бэкап"
    fi

    # [IMP-20] Prometheus metrics
...
S
    ...
    # [IMP-15] Boot recovery service
    if [ "$OPT_BOOT_RECOVERY" = true ]; then
        cat > /usr/local/bin/ha-boot-check << 'S'
#!/bin/bash
sleep 30
dmesg | grep -qi "ext4.*error\|filesystem.*error" && /usr/local/bin/ha-notify "⚠️ FS errors after boot!"
...
UNIT
        systemctl enable ha-boot-check 2>/dev/null; msg_ok "Boot recovery"
    fi

    # [IMP-18] Reverse proxy + SSL
...
do_check() {
    ...
    echo -e "  ${BOLD}Система${NC}"; msg_info "Host: $(hostname 2>/dev/null)  IP: ${ip}  OS: ${CACHED_PRETTY_NAME}"
...
do_uninstall() {
    ...
    for svc in hassio-supervisor hassio-apparmor ha-boot-check; do systemctl disable "$svc" 2>/dev/null; rm -f "/etc/systemd/system/${svc}.service"; done
    ...
    rm -f /usr/local/bin/ha-{notify,watchdog,cleanup,net-recovery,backup,restore,health,thermal,metrics,boot-check,backup-remote} \
          /etc/cron.d/ha-tools /etc/udev/rules.d/99-ha-usb-power.rules /etc/ssh/sshd_config.d/99-ha-hardening.conf \
    ...
    [ -f /etc/nginx/sites-enabled/homeassistant ] && { rm -f /etc/nginx/sites-enabled/homeassistant /etc/nginx/sites-available/homeassistant; systemctl reload nginx 2>/dev/null; }
...
do_update() {
    ...
    if [ "$co" != "$lo" ] && [ -n "$lo" ]; then
        ...
        run_cmd "OS-Agent" dpkg -i "${HA_TMP}/os-agent.deb"; RESOLVED_OA_VER="$lo"
    fi
    if [ "$ch" != "$lh" ] && [ -n "$lh" ]; then
    ...
    RESOLVED_HA_VER="${RESOLVED_HA_VER:-${lh:-$ch}}"; RESOLVED_OA_VER="${RESOLVED_OA_VER:-$lo}"
...
do_self_test() {
    ...
    rm -f "$tsf"; STATE_FILE="$orig_sf"
    # Profile test
    local saved_zram="$OPT_ZRAM"; apply_profile "minimal" 2>/dev/null; [ "$OPT_ZRAM" = true ] && { msg_ok "profile:minimal"; pass=$((pass+1)); } || { msg_error "profile:minimal"; fail=$((fail+1)); }
    OPT_ZRAM="$saved_zram"
    separator
...
parse_args() {
    [ $# -eq 0 ] && return; RUN_WIZARD=false
    while [[ $# -gt 0 ]]; do case "$1" in
        -h|--help) show_help; exit 0;; -c|--check) CHECK_ONLY=true;; -s|--status) SHOW_STATUS=true;;
        -u|--uninstall) UNINSTALL=true;; --update) DO_UPDATE=true;; --self-update) DO_SELF_UPDATE=true;;
        --self-test) DO_SELF_TEST=true;; --reset-state) reset_state; exit 0;;
...
show_banner() {
    [ "$CHECK_ONLY" != true ] && [ "$UNINSTALL" != true ] && [ "$SHOW_STATUS" != true ] && clear
    [ "$SILENT" != true ] && {
        echo -e "${BLUE}    ╦ ╦┌─┐┌┬┐┌─┐  ╔═╗┌─┐┌─┐┬┌─┐┌┬┐┌─┐┌┐┌┌┬┐${NC}"
        echo -e "${BLUE}    ╠═╣│ ││││├┤   ╠═╣└─┐└─┐│└─┐ │ ├─┤│││ │ ${NC}"
        echo -e "${BLUE}    ╩ ╩└─┘┴ ┴└─┘  ╩ ╩└─┘└─┘┴└─┘ ┴ ┴ ┴┘└┘ ┴ ${NC}"
...
show_final() {
    ...
    local el=$(( $(date +%s) - ${INSTALL_START:-$(date +%s)} )) em=$((el/60)) es=$((el%60))
...
main() {
    ...
    # [IMP-19] Apply profile if specified
